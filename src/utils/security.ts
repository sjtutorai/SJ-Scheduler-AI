...
// A pure JavaScript SHA-256 implementation safe for iframe sandboxes
export function hashPassword(password: string): string {
  ...
  const bitLength = l * 8;
  const paddingLength = (l % 64 < 56) ? (56 - (l % 64)) : (120 - (l % 64));
  
  const padded = new Uint8Array(l + paddingLength + 8);
  padded.set(msgBytes);
  padded[l] = 0x80;
